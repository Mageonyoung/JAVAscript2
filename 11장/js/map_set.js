...
//맵은 프로퍼티는 순서대로 접근하고 처리할 수 있다.

let bag =new Map() //Map객체의 인스턴스인 bag생성
console.log(bag.set("색상","빨강"))

// Map객체를 만들면서 프로퍼티를 지정 할 수 있다
// [] 대괄호를 사용하면 처음부터 프로퍼티 지정할 수 있음
let myCup = new Map([
    ["색상","흰색"],
    ["재료","세라믹"],
    ["용량","450ml"]
...
//키나 값을 가져오는 메서드는 이터러블 객체를 반환
for(let key of myCup.keys()){
    console.log(key);
}
for(let values of myCup.values()){
    console.log(values);
}
...
//셋 (set())
//셋은 키가 없다. 여러개의 값을 모아놓은것 -> 배열과 같지만, 값이 중복되지 않는다
let numSet1 = new Set()
numSet1.add(`one`);
numSet1.add(`two`);
console.log(numSet1);
...
//배열을 인수로 받아서 셋으로 만들수 있다.
let numSet2= new Set([1,2,3]);
console.log(numSet2);

let numSet3= new Set([1,2,3,4,5,6,6,7,2,4]);
...
//values 메서드를 사용해 갑을 가져와 for of  반목문으로 
